...
use sled::Db;
use std::{borrow::Borrow, error::Error as StdError, future::Future, sync::Arc, time::Duration};
use tokio::{
...
use crate::proxy::cert::CertifiedKeyDer;
...
impl<I> MitmProxy<I> {
    ...
    pub fn new(root_issuer: I, cert_cache: Db) -> Self {
        Self {
            root_issuer,
            cert_cache,
        }
    ...
    I: Borrow<rcgen::Issuer<'static, rcgen::KeyPair>> + Send + Sync + 'static,
{
        ...
        S::Future: Send,
    {
        ...
        Ok(async move {
                ...
                tokio::select! {
                    ...
                    c = listener.accept() =>{
                        ...
                        tracker.spawn(async move {
                            if let Err(err) = http_mitm_proxy::hyper::server::conn::http1::Builder::new()
                                .preserve_header_case(true)
                                .title_case_headers(true)
                                .serve_connection(
                                    TokioIo::new(stream),
                                    MitmProxy::wrap_service(proxy.clone(), service.clone(), tracker_clone),
                                )
                                .with_upgrades()
                                .await
                            {
            ...
            tracker.close();
            tracing::info!(
                "Stopped accepting new connections. Waiting for active connections to drain (max 10 seconds)..."
            );
            match timeout(Duration::from_secs(10), tracker.wait()).await {
                Ok(_) => tracing::info!("All active connections drained gracefully."),
                Err(_) => tracing::warn!(
                    "Timeout: Some connections did not drain within 10 seconds. Forcing shutdown."
                ),
            }
        })
    }
    ...
    fn get_certified_key(
        &self,
        host: String,
    ) -> Result<sled::IVec, Box<dyn std::error::Error + Send + Sync>> {
        if let Some(ivec) = self.cert_cache.get(&host)? {
            return Ok(ivec);
        }

        let k_vec = CertifiedKeyDer::generate_cert(host.clone(), self.root_issuer.borrow())?;
        let ivec = sled::IVec::from(k_vec);
        self.cert_cache.insert(host, ivec.clone())?;
    ...
    fn server_config(
        ...
        if let Ok(cert) = self.get_certified_key(host) {
            let cert = CertifiedKeyDer(cert.as_ref());
            let config = rustls::ServerConfig::builder()
                ...
                .with_single_cert(
                    ...
                    rustls::pki_types::PrivateKeyDer::Pkcs8(
                        rustls::pki_types::PrivatePkcs8KeyDer::from(cert.key_der().to_vec()),
                    ),
                );
